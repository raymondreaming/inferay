...
mod tests {
    ...
    fn tree_hash_is_deterministic_and_honors_exclusions() {
        ...
        for (name, mode) in [("a.txt", 0o640), ("z.txt", 0o644)] {
            fs::set_permissions(temp.path().join(name), fs::Permissions::from_mode(mode)).unwrap();
        }
        std::os::unix::fs::symlink("a.txt", temp.path().join("alias")).unwrap();

        let first = hash_tree(temp.path(), &["Contents/Resources/version.json"]).unwrap();
        assert_eq!(first, "e8da473b4a5c");
