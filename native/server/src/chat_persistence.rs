...
mod group_commit_tests {
    ...
    #[tokio::test]
    async fn concurrent_panes_keep_order_and_acknowledged_updates_survive_reopen() {
        let root = tempfile::tempdir().unwrap();
        let store = ChatPersistence::new(root.path().into());
                    ...
                    store
                        .enqueue_runtime(
                            ...
                            QueuedMessageInfo {
                                ...
                                display_text: format!("message-{index}"),
                                images: None,
                            },
                        )
                        .await
        ...
        drop(store);
        let reopened = ChatPersistence::new(root.path().into());
        for pane in 0..6 {
            let queue = reopened.read_queue(&format!("pane-{pane}")).await.unwrap();
            assert_eq!(queue.len(), 20);
            for (index, message) in queue.iter().enumerate() {
                assert_eq!(message.id, index.to_string());
            }
        }
    }
}
