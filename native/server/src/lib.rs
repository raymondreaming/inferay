...
struct ServerState {
    ...
    simulator_baguette_process: Arc<tokio::sync::Mutex<Option<tokio::process::Child>>>,
...
fn build_router_with_connection_reset(
    ...
    let state = ServerState {
        ...
        native_prompts,
        simulator_baguette_process: Arc::new(tokio::sync::Mutex::new(None)),
        pid_tracker,
...
async fn dispatch_request(State(state): State<ServerState>, request: Request) -> Response {
    ...
    if path.starts_with("/api/") || path == "/api" {
        ...
        if cfg!(target_os = "macos") && path == "/api/simulator/project-folders" {
            if request.method() == Method::GET {
                return get_simulator_project_folders(&state, request).await;
            }
            if request.method() == Method::PUT {
                return update_simulator_project_folders(&state, request).await;
            }
        }
        if cfg!(target_os = "macos")
            && path == "/api/simulator/project-folders/pick"
            && request.method() == Method::POST
        {
            return pick_simulator_project_folder(&state, request).await;
        }
        if cfg!(target_os = "macos")
            && path == "/api/simulator/project-folders/detect"
            && request.method() == Method::POST
        {
            return detect_simulator_project_folders(&state, request).await;
        }
        if cfg!(target_os = "macos") && simulator::is_route(&path, request.method()) {
            let service = simulator::SimulatorService::new(Arc::new(
                simulator::ServerSimulatorBackend::new(state.clone()),
            ));
            return simulator::handle_request(&service, &path, request).await;
        }
...
async fn get_simulator_project_folders(state: &ServerState, request: Request) -> Response {
    let request_headers = request.headers().clone();
    let folders = simulator_project_folders(state).await;
    json_response(
        StatusCode::OK,
...
async fn update_simulator_project_folders(state: &ServerState, request: Request) -> Response {
    let request_headers = request.headers().clone();
    let body: Value = match request_json(request, &request_headers).await {
        Ok(body) => body,
        Err(response) => return response,
    };
    let Some(folders) = body.get("folders").and_then(Value::as_array) else {
        return json_response(
            StatusCode::BAD_REQUEST,
            json!({ "ok": false, "error": "folders must be an array" }),
            &request_headers,
        );
    };
    let folders = folders
        ...
        .map(str::to_string)
        .collect::<Vec<_>>();
    match set_simulator_project_folders(state, folders).await {
        ...
        Err(error) => {
            simulator_error_response(error, StatusCode::INTERNAL_SERVER_ERROR, &request_headers)
        }
    }
}

async fn pick_simulator_project_folder(state: &ServerState, request: Request) -> Response {
    let request_headers = request.headers().clone();
    let folder = selected_simulator_project_folder()
        .await
        .map(|folder| simulator_display_path(state.allowed_paths.home_directory(), &folder));
    json_response(
        StatusCode::OK,
...
async fn detect_simulator_project_folders(state: &ServerState, request: Request) -> Response {
    let request_headers = request.headers().clone();
    match auto_detect_simulator_project_folders(state).await {
        Ok(folders) => json_response(
            StatusCode::OK,
            json!({ "ok": true, "folders": folders }),
            &request_headers,
...
fn simulator_error_response(error: String, status: StatusCode, headers: &HeaderMap) -> Response {
    json_response(status, json!({ "ok": false, "error": error }), headers)
}

async fn simulator_project_folders(state: &ServerState) -> Vec<String> {
    state
        .config_manager
        .lock()
        .await
...
async fn set_simulator_project_folders(
    state: &ServerState,
    folders: Vec<String>,
) -> Result<Vec<String>, String> {
    let normalized = unique_trimmed_strings(
        folders
            .into_iter()
            .map(|folder| simulator_display_path(state.allowed_paths.home_directory(), &folder)),
    );
    let config = state.config_manager.lock().await.update(
        json!({ "simulator_project_folders": normalized })
            .as_object()
            .expect("simulator folder update must be an object")
            .clone(),
    )?;
    Ok(config
        .get("simulator_project_folders")
...
async fn auto_detect_simulator_project_folders(state: &ServerState) -> Result<Vec<String>, String> {
    ...
    let candidates = unique_trimmed_strings(
        [
            "~/Desktop".to_string(),
            "~/Desktop/Code".to_string(),
            "~/Desktop/HatchingPoint".to_string(),
            "~/Desktop/RealityDesigners".to_string(),
            "~/Code".to_string(),
            "~/Developer".to_string(),
        ]
        .into_iter()
    ...
    let detected = tokio::task::spawn_blocking(move || {
        ...
        for root in candidates {
            let expanded = expand_simulator_home(&home, &root);
            if !expanded.is_dir() {
                continue;
            }
            for target in find_simulator_build_targets(&expanded, 5) {
                let display = simulator_display_path(&home, &target.path.to_string_lossy());
                if seen.insert(display.clone()) {
                    detected.push(display);
                }
            }
        }
        detected
    ...
    .await
    .map_err(|error| error.to_string())?;
    let merged = unique_trimmed_strings(
        simulator_project_folders(state)
            .await
            .into_iter()
            .chain(detected),
    );
    set_simulator_project_folders(state, merged).await
}

async fn simulator_workspace_paths(state: &ServerState) -> Vec<String> {
    let data = state
        .agent_state_store
        .lock()
        .expect("agent state lock poisoned")
    ...
    for group in groups {
        ...
        for pane in panes {
            if let Some(cwd) = pane.get("cwd").and_then(Value::as_str)
                && !cwd.is_empty()
                && seen.insert(cwd.to_string())
            {
                paths.push(cwd.to_string());
            }
            if let Some(references) = pane.get("referencePaths").and_then(Value::as_array) {
...
fn find_simulator_build_targets(root: &Path, depth: isize) -> Vec<SimulatorBuildTarget> {
    let mut found = Vec::<SimulatorBuildTarget>::new();
    scan_simulator_build_targets(root, depth, &mut found);
    found
}

fn scan_simulator_build_targets(
    directory: &Path,
    remaining: isize,
    found: &mut Vec<SimulatorBuildTarget>,
) {
    if remaining < 0 {
        return;
    }
    ...
    for entry in entries.flatten() {
        ...
        if name.ends_with(".xcodeproj") || name.ends_with(".xcworkspace") {
            add_simulator_build_target(directory, &full_path, &name, found);
            continue;
        }
        if [
            ".git",
        ...
        scan_simulator_build_targets(&full_path, remaining - 1, found);
    }
}

fn add_simulator_build_target(
    directory: &Path,
    full_path: &Path,
    name: &str,
    found: &mut Vec<SimulatorBuildTarget>,
) {
    let app_root = if directory.file_name().and_then(|name| name.to_str()) == Some("ios") {
        directory.parent().unwrap_or(directory)
    } else {
        directory
    };
    let index = found.iter().position(|target| target.path == app_root);
    let target = if let Some(index) = index {
        &mut found[index]
    } else {
        found.push(SimulatorBuildTarget {
            path: app_root.to_path_buf(),
            project_path: None,
            workspace_path: None,
            ios_path: None,
        });
        found.last_mut().expect("inserted simulator target")
    };
    if name.ends_with(".xcworkspace") {
        target.workspace_path = Some(full_path.to_path_buf());
        target.ios_path = Some(directory.to_path_buf());
    } else if target.project_path.is_none() {
        target.project_path = Some(full_path.to_path_buf());
        if directory.file_name().and_then(|name| name.to_str()) == Some("ios") {
            target.ios_path = Some(directory.to_path_buf());
        }
    }
}

fn expand_simulator_home(home: &Path, path: &str) -> PathBuf {
    if path == "~" {
        return home.to_path_buf();
    }
    path.strip_prefix("~/")
        .map_or_else(|| PathBuf::from(path), |path| home.join(path))
}

fn simulator_display_path(home: &Path, path: &str) -> String {
    let cleaned = path.trim_end_matches('/');
    let home = home.to_string_lossy();
    cleaned
        .strip_prefix(&format!("{home}/"))
        .map_or_else(|| cleaned.to_string(), |path| format!("~/{path}"))
}

fn unique_trimmed_strings(values: impl IntoIterator<Item = String>) -> Vec<String> {
...
async fn selected_simulator_project_folder() -> Option<String> {
    #[cfg(target_os = "macos")]
    {
        let output = tokio::time::timeout(
            ...
            tokio::process::Command::new("osascript")
                .args([
                    "-e",
                    "POSIX path of (choose folder with prompt \"Select an Xcode or React Native project folder\")",
                ])
                .output(),
        )
        .await
...
"#,
    ...
    #[cfg(target_os = "macos")]
    #[tokio::test]
    async fn manages_and_detects_simulator_project_folders_without_bun() {
        let root = TempDir::new().unwrap();
        let mobile = root.path().join("projects/MobileApp");
        std::fs::create_dir_all(mobile.join("ios/MobileApp.xcodeproj")).unwrap();
        std::fs::create_dir_all(mobile.join("ios/MobileApp.xcworkspace")).unwrap();
        let app = router(test_config(root.path()));

        let folder = root.path().join("projects").to_string_lossy().into_owned();
        let display_folder = "~/projects";
        let (status, value) = call_json(
            &app,
            Method::PUT,
            "/api/simulator/project-folders".into(),
            Some(json!({ "folders": [format!("  {folder}/  "), folder, 7] })),
        )
        .await;
        assert_eq!(status, StatusCode::OK);
        assert_eq!(
            value,
            json!({ "ok": true, "folders": [format!("{folder}/"), display_folder] })
        );
        ...
        let (status, _) = call_json(
            ...
            Method::PUT,
            "/api/config/search-folders".into(),
            Some(json!({ "folders": [root.path().join("projects")] })),
        )
        .await;
        assert_eq!(status, StatusCode::OK);
        let (status, value) = call_json(
            &app,
            Method::POST,
            "/api/simulator/project-folders/detect".into(),
            Some(json!({})),
        )
        .await;
        assert_eq!(status, StatusCode::OK);
        ...
        assert_eq!(
            value["folders"],
            json!([display_folder, "~/projects/MobileApp"])
        );

        let targets = find_simulator_build_targets(&root.path().join("projects"), 5);
        assert_eq!(targets.len(), 1);
        assert_eq!(targets[0].path, mobile);
        assert!(
            targets[0]
                .project_path
                .as_ref()
                .unwrap()
                .ends_with("MobileApp.xcodeproj")
        );
        assert!(
            targets[0]
                .workspace_path
                .as_ref()
                .unwrap()
                .ends_with("MobileApp.xcworkspace")
        );
        assert!(targets[0].ios_path.as_ref().unwrap().ends_with("ios"));

        let (status, value) = call_json(
            &app,
            Method::PUT,
            "/api/simulator/project-folders".into(),
            Some(json!({ "folders": "not-an-array" })),
        )
        .await;
        assert_eq!(status, StatusCode::BAD_REQUEST);
        assert_eq!(
    ...
    #[cfg(target_os = "macos")]
    #[tokio::test]
    async fn validates_native_simulator_actions_without_running_xcode_tools() {
        let root = TempDir::new().unwrap();
        let app = router(test_config(root.path()));

        for (path, key) in [
            ("/api/simulator/boot", "udid"),
            ("/api/simulator/shutdown", "udid"),
            ("/api/simulator/open-xcode", "appPath"),
            ("/api/simulator/build-launch", "udid"),
        ] {
            let (status, value) = call_json(&app, Method::POST, path.into(), Some(json!({}))).await;
            assert_eq!(status, StatusCode::BAD_REQUEST, "{path}");
            assert_eq!(
                value,
                json!({ "ok": false, "error": format!("{key} required") }),
                "{path}"
            );
        }

        let outside = root.path().parent().unwrap().join("outside-project");
        let (status, value) = call_json(
            &app,
            Method::POST,
            "/api/simulator/open-xcode".into(),
            Some(json!({ "appPath": outside })),
        )
        .await;
        assert_eq!(status, StatusCode::NOT_FOUND);
        assert_eq!(
            value,
            json!({ "ok": false, "error": "No .xcodeproj found" })
        );

        let (status, value) = call_json(
            &app,
            Method::POST,
            "/api/simulator/build-launch".into(),
            Some(json!({ "udid": "test-device", "appPath": outside })),
        )
        .await;
        assert_eq!(status, StatusCode::INTERNAL_SERVER_ERROR);
        assert_eq!(
            ...
            json!({
                "ok": false,
                "error": "Project path is outside allowed local roots"
            })
        );
    }
